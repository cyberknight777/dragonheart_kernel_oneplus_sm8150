...
static void g(struct streebog_uint512 *h, const struct streebog_uint512 *N,
	      const u8 *m)
{
	...
	/* Starting E() */
	Ki = data;
	XLPS(&Ki, (const struct streebog_uint512 *)&m[0], &data);
	...
	X(&data, h, &data);
	X(&data, (const struct streebog_uint512 *)&m[0], h);
}

static inline void stage2(struct streebog_state *ctx, const u8 *data)
{
	g(&ctx->h, &ctx->N, data);

	add512(&ctx->N, &buffer512, &ctx->N);
	add512(&ctx->Sigma, (const struct streebog_uint512 *)data, &ctx->Sigma);
}
...
static inline void stage3(struct streebog_state *ctx)
{
	...
	buf.qword[0] = cpu_to_le64(ctx->bufsize << 3);
	pad(ctx);

	g(&ctx->h, &ctx->N, (const u8 *)&ctx->buffer);

	add512(&ctx->N, &buf, &ctx->N);
	add512(&ctx->Sigma, (const struct streebog_uint512 *)&ctx->buffer[0],
	       &ctx->Sigma);

	g(&ctx->h, &buffer0, (const u8 *)&ctx->N);

	g(&ctx->h, &buffer0, (const u8 *)&ctx->Sigma);
