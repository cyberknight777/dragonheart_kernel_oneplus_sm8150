				   ...
				   unsigned long mode, void *cmd)
{
	...
	if (rc) {
		pr_debug("failed to write rst_src_en (%d)\n", rc);
		return NOTIFY_DONE;
	}

	rc = regmap_read(regmap, rst_src_en, &tmp);
	if (rc) {
		pr_debug("failed to read rst_src_en (%d)\n", rc);
		return NOTIFY_DONE;
	}

	rc = regmap_write(regmap, sw_mstr_rst, reset_masks->sw_mstr_rst_mask);
	if (rc) {
		pr_debug("failed to write sw_mstr_rst (%d)\n", rc);
		return NOTIFY_DONE;
	}

	rc = regmap_read(regmap, sw_mstr_rst, &tmp);
	if (rc) {
		pr_debug("failed to read sw_mstr_rst (%d)\n", rc);
		return NOTIFY_DONE;
	}
...
static int brcmstb_reboot_probe(struct platform_device *pdev)
{
	...
	if (!of_id) {
		pr_debug("failed to look up compatible string\n");
		return -EINVAL;
	}
	...
	if (IS_ERR(regmap)) {
		pr_debug("failed to get syscon phandle\n");
		return -EINVAL;
	}
	...
	rc = of_property_read_u32_index(np, "syscon", RESET_SOURCE_ENABLE_REG,
					&rst_src_en);
	if (rc) {
		pr_debug("can't get rst_src_en offset (%d)\n", rc);
		return -EINVAL;
	}
	...
	rc = of_property_read_u32_index(np, "syscon", SW_MASTER_RESET_REG,
					&sw_mstr_rst);
	if (rc) {
		pr_debug("can't get sw_mstr_rst offset (%d)\n", rc);
		return -EINVAL;
	}
