...
static int fg_gen4_hw_init(struct fg_gen4_chip *chip)
{
	...
	if (rc < 0) {
		pr_debug("Error in writing ESR PULL DN, rc=%d\n", rc);
		return rc;
	}
				...
				bool status)
{
	if (chip->use_external_fg && external_fg
		...
		pr_debug("set allow read extern fg iic fail\n");
}
				...
				bool status)
{
	if (chip->use_external_fg && external_fg
		...
		pr_debug("set lcd off status fail\n");
}
...
static int fg_gen4_probe(struct platform_device *pdev)
{
	...
	pr_debug("yfb FG GEN4 driver probed successfully\n");
	return 0;
exit:
