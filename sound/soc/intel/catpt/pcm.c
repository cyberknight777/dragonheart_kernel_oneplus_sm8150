...
#define DRV_NAME "catpt-platform"
...
catpt_get_stream_template(struct snd_pcm_substream *substream)
{
	...
	struct snd_soc_dai *cpu_dai = rtm->cpu_dai;
	enum catpt_stream_type type;
...
static int catpt_component_pcm_new(struct snd_soc_pcm_runtime *rtm)
{
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtm, DRV_NAME);
	struct catpt_dev *cdev = dev_get_drvdata(component->dev);

	return snd_pcm_lib_preallocate_pages_for_all(rtm->pcm,
				       SNDRV_DMA_TYPE_DEV_SG, cdev->dev,
				       catpt_pcm_hardware.buffer_bytes_max,
				       catpt_pcm_hardware.buffer_bytes_max);
}

static void catpt_component_pcm_free(struct snd_pcm *pcm)
{
	snd_pcm_lib_preallocate_free_for_all(pcm);
}

static int catpt_component_open(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtm = substream->private_data;
...
catpt_component_pointer(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtm = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtm, DRV_NAME);
...
static int catpt_dai_pcm_new(struct snd_soc_pcm_runtime *rtm,
			     struct snd_soc_dai *dai)
{
	struct snd_soc_dai *codec_dai = rtm->codec_dai;
	struct catpt_dev *cdev = dev_get_drvdata(dai->dev);
...
static const struct snd_pcm_ops catpt_pcm_ops = {
...
static const struct snd_soc_component_driver catpt_comp_driver = {
	...
	.pcm_free = catpt_component_pcm_free,
	.ops = &catpt_pcm_ops,
...
int catpt_register_plat_component(struct catpt_dev *cdev)
{
	...
	component->name = kstrdup(catpt_comp_driver.name, GFP_KERNEL);
	if (!component->name)
		return -ENOMEM;
